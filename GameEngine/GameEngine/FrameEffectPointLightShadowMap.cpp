...
void FrameEffectPointLightShadowMap::Setup(IDXGISwapChain* swapChain, ID3D11Device* device, const D3D11_VIEWPORT& viewPort, bool msaa, FrameBufferName name, bool shadow, bool depth)
{
	HRESULT hr = S_OK;
	...
	texture2d_desc.ArraySize = slideCount;
	...
	depth_stencil_view_desc.Texture2DArray.ArraySize = static_cast<UINT>(slideCount);
	...
	shader_resource_view_desc.Texture2DArray.ArraySize = static_cast<UINT>(slideCount);
...
void FrameEffectPointLightShadowMap::DrawedOn(ID3D11DeviceContext* immediateContext)
{
	...
	for (size_t i = 0; i < lightManager->getSizePointLightList(); i++)
	{
		...
		shaderManager->BeginPointLightCastShadow(immediateContext);
		GameEngine::get()->CastShadow(FrameBufferName::MSAABUFFER1, DrawType::SkinnedMeshObject, slideCount);
...
void FrameEffectPointLightShadowMap::DrawToDebug()
{

}

FrameEffectPointLightShadowMap::~FrameEffectPointLightShadowMap()
{
